...
local battery_style = (MODULES.battery and MODULES.battery.enable ~= false)
    and (MODULES.battery.style or 'icon')
  or nil
...
local function update_wifi()
  ...
  SBAR.exec("networksetup -getairportpower en0 2>/dev/null | awk '{print $NF}'", function(pwr)
    pwr = (pwr or ''):gsub('\n$', '')
    if pwr == 'Off' then
      -- Power off: show only icon, hide text for all styles
      wifi_item:set({
        icon = { string = ICONS.wifi.off, drawing = true, color = COLORS.overlay0 },
        label = { drawing = false },
      })
      return
    end
