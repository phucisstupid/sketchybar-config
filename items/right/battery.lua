      ...
      local function update_popup_info()
        SBAR.exec('pmset -g batt', function(batt_info)
          ...
          if remaining_match then
            ...
            SBAR.exec(
              "ioreg -rn AppleSmartBattery | grep -E '(TimeRemaining|InstantTimeToEmpty)' | head -1 | awk '{print $3}'",
              function(ioreg_result)
                if ioreg_result and ioreg_result:match('%d+') then
                  ...
                  if minutes and minutes > 0 then
                    ...
                    if hours > 0 then
                      ...
                      remaining_time:set({
                        label = { string = string.format('%d:%02d', hours, mins) },
                      })
                    else
